    ...
    def Driving(self):
        ...
        while not exit:
            if self.context.obstacle.is_triggered():
                ...
                #
                if self.context.obs == Direction.FRONT:
                    next = random.choices([Direction.LEFT,Direction.RIGHT], [0.4, 0.6], k=1)[0]
                    if next == Direction.LEFT:
                        yield from self.context.move(0.0, -avel)
                    elif next == Direction.RIGHT:
                        yield from self.context.move(0.0, avel)
                elif self.context.obs == Direction.RIGHT:
                    yield from self.context.move(0.0, avel)
