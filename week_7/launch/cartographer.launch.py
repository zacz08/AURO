...
def generate_launch_description():
    ...
    declare_cartographer_config = DeclareLaunchArgument(
        'cartographer_config',
        default_value=os.path.join(get_package_share_directory('assessment'), 'config'),
        description="Folder where configuration file 'turtlebot3_lds_2d.lua' is expected to be found to be configure the cartographer"
    )
    ...
    return LaunchDescription([
            ...
            IncludeLaunchDescription(
                ...
                launch_arguments={'use_rviz': 'false', 'use_sim_time': use_sim_time, 'cartographer_config_dir': os.path.join(get_package_share_directory('week_7'), 'config')}.items()
            )
        ])